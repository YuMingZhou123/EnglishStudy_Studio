...
后台内容写入会复用固定的 smoke 场景、单词和句子，重复执行不会持续增加测试场景、测试单词和测试句子。
固定 smoke 句子已有音频时，`-IncludeTts` 会复用并回读该音频，避免重复生成 TTS 资产。
...
脚本同时覆盖桌面视口和 390px 移动端视口，并检查核心页面没有明显横向溢出。
...
生成的 `acceptance/first-version-human-plan.md` 会按顺序列出内容审核、真实内测和最终发布门禁的人工执行队列。
